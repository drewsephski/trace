...
const OUTPUT_DIR =
	process.env.CURSOR_TEST_OUTPUT_DIR ?? path.join(os.tmpdir(), `trace-cursor-native-${Date.now()}`);
...
function spawnPowerShell(script, { onStdout, onStderr } = {}) {
	const scriptPath = path.join(
		os.tmpdir(),
		`trace-powershell-${process.pid}-${Date.now()}-${Math.random().toString(16).slice(2)}.ps1`,
	);
...
public static class TraceCursorDiagnosticInterop {
...
$standardCursors = @{
    arrow = [TraceCursorDiagnosticInterop]::LoadCursor([IntPtr]::Zero, [IntPtr]::new(32512))
    text = [TraceCursorDiagnosticInterop]::LoadCursor([IntPtr]::Zero, [IntPtr]::new(32513))
    wait = [TraceCursorDiagnosticInterop]::LoadCursor([IntPtr]::Zero, [IntPtr]::new(32514))
    crosshair = [TraceCursorDiagnosticInterop]::LoadCursor([IntPtr]::Zero, [IntPtr]::new(32515))
    'up-arrow' = [TraceCursorDiagnosticInterop]::LoadCursor([IntPtr]::Zero, [IntPtr]::new(32516))
    'resize-nwse' = [TraceCursorDiagnosticInterop]::LoadCursor([IntPtr]::Zero, [IntPtr]::new(32642))
    'resize-nesw' = [TraceCursorDiagnosticInterop]::LoadCursor([IntPtr]::Zero, [IntPtr]::new(32643))
    'resize-ew' = [TraceCursorDiagnosticInterop]::LoadCursor([IntPtr]::Zero, [IntPtr]::new(32644))
    'resize-ns' = [TraceCursorDiagnosticInterop]::LoadCursor([IntPtr]::Zero, [IntPtr]::new(32645))
    move = [TraceCursorDiagnosticInterop]::LoadCursor([IntPtr]::Zero, [IntPtr]::new(32646))
    'not-allowed' = [TraceCursorDiagnosticInterop]::LoadCursor([IntPtr]::Zero, [IntPtr]::new(32648))
    pointer = [TraceCursorDiagnosticInterop]::LoadCursor([IntPtr]::Zero, [IntPtr]::new(32649))
    'app-starting' = [TraceCursorDiagnosticInterop]::LoadCursor([IntPtr]::Zero, [IntPtr]::new(32650))
    help = [TraceCursorDiagnosticInterop]::LoadCursor([IntPtr]::Zero, [IntPtr]::new(32651))
}
...
function Get-CursorAsset($cursorHandle, $cursorId) {
    $copiedHandle = [TraceCursorDiagnosticInterop]::CopyIcon($cursorHandle)
    if ($copiedHandle -eq [IntPtr]::Zero) {
        return $null
    }

    $iconInfo = New-Object TraceCursorDiagnosticInterop+ICONINFO
    $hasIconInfo = [TraceCursorDiagnosticInterop]::GetIconInfo($copiedHandle, [ref]$iconInfo)
    ...
    try {
        ...
        if ($hasIconInfo) {
            if ($iconInfo.hbmMask -ne [IntPtr]::Zero) {
                [TraceCursorDiagnosticInterop]::DeleteObject($iconInfo.hbmMask) | Out-Null
            }
            if ($iconInfo.hbmColor -ne [IntPtr]::Zero) {
                [TraceCursorDiagnosticInterop]::DeleteObject($iconInfo.hbmColor) | Out-Null
            }
        }
        [TraceCursorDiagnosticInterop]::DestroyIcon($copiedHandle) | Out-Null
    }
}

[TraceCursorDiagnosticInterop]::InstallMouseHook() | Out-Null
[TraceCursorDiagnosticInterop]::GetAsyncKeyState(0x01) | Out-Null
...
while ($true) {
    ...
    $mouseEvents = [TraceCursorDiagnosticInterop]::ConsumeMouseButtonEvents()
    $cursorInfo = New-Object TraceCursorDiagnosticInterop+CURSORINFO
    $cursorInfo.cbSize = [Runtime.InteropServices.Marshal]::SizeOf([type][TraceCursorDiagnosticInterop+CURSORINFO])

    if (-not [TraceCursorDiagnosticInterop]::GetCursorInfo([ref]$cursorInfo)) {
    ...
    $leftButtonState = [TraceCursorDiagnosticInterop]::GetAsyncKeyState(0x01)
...
public static class TraceMouseDiagnosticInterop {
    [DllImport("user32.dll")]
    [return: MarshalAs(UnmanagedType.Bool)]
...
for ($i = 0; $i -lt $points.Count; $i++) {
    $point = $points[$i]
    [TraceMouseDiagnosticInterop]::SetCursorPos($point.x, $point.y) | Out-Null
    if ($i -eq [int]([Math]::Floor($points.Count / 2))) {
        [TraceMouseDiagnosticInterop]::mouse_event(0x0002, 0, 0, 0, [UIntPtr]::Zero)
        Start-Sleep -Milliseconds 12
        [TraceMouseDiagnosticInterop]::mouse_event(0x0004, 0, 0, 0, [UIntPtr]::Zero)
    }
...
<title>Trace native cursor diagnostic</title>
<style>
...
<body>
<main>
<h1>Trace native cursor diagnostic</h1>
<div class="metrics">
...
<meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<title>Trace native cursor real capture diagnostic</title>
<style>
