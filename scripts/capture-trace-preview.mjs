...
const OUTPUT_DIR =
	process.env.TRACE_PREVIEW_OUTPUT_DIR ??
	path.join(os.tmpdir(), `trace-real-preview-${Date.now()}`);
const FRAME_COUNT = Number(process.env.TRACE_PREVIEW_FRAME_COUNT ?? 90);
const FPS = Number(process.env.TRACE_PREVIEW_FPS ?? 30);
...
function findLatestCursorRecordingData() {
	...
	const candidates = fs
		...
		.filter((entry) => entry.isDirectory() && entry.name.startsWith("trace-cursor-native-"))
...
function runNpmBuildViteIfRequested() {
	if (process.env.TRACE_PREVIEW_SKIP_BUILD === "true") {
		ensureBuildExists();
...
const fixtureVideoPath = path.join(OUTPUT_DIR, "trace-preview-fixture.webm");
const outputVideoPath = path.join(OUTPUT_DIR, "trace-preview.webm");
...
try {
	...
	await hudWindow.evaluate(async () => {
		...
		throw new Error("Timed out waiting for Trace IPC handlers.");
	});
