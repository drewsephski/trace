...
This script does not launch Trace. It:
...
```text
C:\Users\<user>\AppData\Local\Temp\trace-cursor-native-...
```
...
$env:CURSOR_TEST_OUTPUT_DIR = "C:\temp\trace-cursor-test"
npm run test:cursor-native:win
```

## Trace preview capture

```powershell
npm run capture:trace-preview
```

This script launches the real Electron app, injects a fixture video plus cursor sidecar data, opens the editor, captures frames from the actual Trace preview UI, and encodes them into a WebM.
...
```powershell
$env:TRACE_PREVIEW_SKIP_BUILD = "true"
$env:TRACE_PREVIEW_FRAME_COUNT = "120"
$env:TRACE_PREVIEW_FPS = "30"
$env:TRACE_PREVIEW_OUTPUT_DIR = "C:\temp\trace-preview"
npm run capture:trace-preview
```

Useful files:

- `trace-preview.webm`: video of the real Trace editor preview
...
- whether the real Trace preview renders the same cursor behavior as the diagnostic pipeline
...
For local diagnostics with another compatible helper, point Trace at that executable:

```powershell
$env:TRACE_WGC_CAPTURE_EXE = "C:\path\to\wgc-capture.exe"
npm run build-vite
npm run dev
```
