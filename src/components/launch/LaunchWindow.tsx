...
export function LaunchWindow() {
	...
	const startupPromptRef = useRef<HTMLDivElement | null>(null);
	const deviceSelectorRef = useRef<HTMLDivElement | null>(null);
	const [showStartupPrompt, setShowStartupPrompt] = useState(true);
	const [showSourceHint, setShowSourceHint] = useState(true);
	const sourceBtnRef = useRef<HTMLButtonElement | null>(null);
	const [hintPos, setHintPos] = useState<{ top: number; left: number; arrowDown: boolean } | null>(
		null,
	);
	...
	const measureHudSize = useCallback(() => {
		...
		const startupPromptEl = startupPromptRef.current;
		const primaryEl = startupPromptEl ?? barEl;
		if (!primaryEl || !window.electronAPI?.setHudOverlaySize) return;
		...
		let topFromBottom =
			viewportHeight - primaryEl.getBoundingClientRect().bottom + primaryEl.scrollHeight;
		let halfWidth = primaryEl.scrollWidth / 2;
		...
		setHudBarHeight((prev) => {
			const next = Math.round(barEl?.scrollHeight ?? 0);
	...
	useEffect(() => {
		...
		if (startupPromptRef.current) observer.observe(startupPromptRef.current);
	...
	const setStartupPromptEl = useCallback(
		(el: HTMLDivElement | null) => observeHudElement(el, startupPromptRef),
		[observeHudElement],
	);
	...
	useEffect(() => {
		setHudMouseEventsEnabled(isLanguageMenuOpen || showStartupPrompt);
	}, [isLanguageMenuOpen, showStartupPrompt, setHudMouseEventsEnabled]);
	...
	useEffect(() => {
		if (showSourceHint && !hasSelectedSource) {
			const updatePos = () => {
				if (sourceBtnRef.current) {
					const rect = sourceBtnRef.current.getBoundingClientRect();
					const gap = 8;
					const hintEstimate = 44;
					const centerX = rect.left + rect.width / 2;
					const clampedLeft = Math.max(12, Math.min(window.innerWidth - 12, centerX));
					const spaceAbove = rect.top - gap - hintEstimate;
					if (spaceAbove > 0) {
						setHintPos({
							top: spaceAbove,
							left: clampedLeft,
	...
	}, [showSourceHint, hasSelectedSource]);
	...
	return (
		...
		<div
			...
			onPointerLeave={() => {
				if (!isLanguageMenuOpen && !showStartupPrompt) {
					setHudMouseEventsEnabled(false);
				}
			}}
		>
			{showStartupPrompt && (
				<div
					ref={setStartupPromptEl}
					data-hud-interactive="true"
					className={`fixed bottom-5 left-1/2 z-40 flex w-[min(260px,calc(100vw-1rem))] -translate-x-1/2 rounded-2xl border border-white/[0.12] bg-[#07080a]/95 p-2 text-white shadow-[0_20px_60px_rgba(0,0,0,0.42),inset_0_1px_0_rgba(255,255,255,0.06)] backdrop-blur-2xl backdrop-saturate-[140%] ${styles.electronNoDrag}`}
				...
				>
					<button
						type="button"
						data-testid="launch-startup-start-button"
						className="flex h-10 w-full items-center justify-center rounded-xl bg-white px-4 text-sm font-semibold text-[#090a0d] transition-colors hover:bg-white/90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/55"
						onClick={() => {
							setShowStartupPrompt(false);
							requestAnimationFrame(measureHudSize);
						}}
					>
						Start Recording
					</button>
			...
			{!showStartupPrompt && systemLocaleSuggestion && (
				<div
					data-hud-interactive="true"
			...
			{!showStartupPrompt && (showMicControls || showWebcamControls) && (
				<div
			...
			{!showStartupPrompt && (
				<>
					...
					{hintPos && showSourceHint && !hasSelectedSource && (
						<div
							data-hud-interactive="true"
							...
							onMouseEnter={() => setHudMouseEventsEnabled(true)}
						>
							<div className="relative -translate-x-1/2 animate-in fade-in-0 slide-in-from-bottom-1 duration-200">
								<div className="flex items-center gap-2 whitespace-nowrap rounded-lg border border-white/[0.10] bg-[#07080a]/95 px-3 py-1.5 text-[11px] text-white/75 shadow-[0_8px_24px_rgba(0,0,0,0.35)]">
									<span>{t("recording.selectSourceHint")}</span>
									<button
										type="button"
										onClick={(e) => {
											e.stopPropagation();
										...
										className="flex items-center justify-center rounded p-0.5 text-white/30 hover:text-white/80 hover:bg-white/10 transition-colors"
								...
								{hintPos.arrowDown ? (
									<div className="absolute left-1/2 top-full -translate-x-1/2 h-0 w-0 border-l-[6px] border-r-[6px] border-t-[6px] border-l-transparent border-r-transparent border-t-[#07080a]" />
								) : (
									<div className="absolute left-1/2 bottom-full -translate-x-1/2 h-0 w-0 border-l-[6px] border-r-[6px] border-b-[6px] border-l-transparent border-r-transparent border-b-[#07080a]" />
								)}
							</div>
						</div>
					)}
