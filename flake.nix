{
  description = "Trace — desktop screen recorder with built-in editor";
  ...
  outputs =
    ...
    in
    {
      ...
      packages = forAllSystems (pkgs: {
        trace = pkgs.callPackage ./nix/package.nix { };
      ...
      devShells = forAllSystems (
        ...
        in
        {
          default = pkgs.mkShell {
            ...
            shellHook = ''
              echo "Trace dev shell — node $(node --version), electron v$(electron --version 2>/dev/null | tr -d 'v')"
            '';
          };
        }
      );
      ...
      overlays.default = final: _prev: {
        trace = self.packages.${final.stdenv.hostPlatform.system}.trace;
      };
