...
import { AuthTokenResponse, Session, SupabaseClient, User } from "@supabase/supabase-js";
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
	...
	const value = {
		...
		signInWithGoogle: async (redirectPath?: string) => {
			const redirectSuffix = redirectPath ? `?next=${encodeURIComponent(redirectPath)}` : "";
		...
		deleteAccount: async () => {
			...
			const { error: dataError } = await supabase.from("users").delete().eq("id", user?.id);
